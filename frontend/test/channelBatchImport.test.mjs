...
// 修改原因：Channels.tsx 拆分后，API Keys 标题栏位于 ChannelEditor.tsx 的「2. API Keys」区块。
// 修改方式：以区块注释和数据滚动容器为边界截取标题栏源码。
const headerStart = editorSource.indexOf('{/* 2. API Keys');
const headerEnd = editorSource.indexOf('<div data-key-scroll');
assert.notEqual(headerStart, -1, '找不到 API Keys 区块起始');
assert.notEqual(headerEnd, -1, '找不到 Key 滚动容器');
...
assert.match(editorSource, /document\.addEventListener\('mousedown', handler\)/, '更多菜单应支持点击外部关闭');
...
assert.match(editorSource, /\{formData\.api_keys\.length === 0 && \(/, '0 key 时应渲染空状态操作区');
assert.match(editorSource, /暂无密钥/, '空状态应显示“暂无密钥”');
assert.match(editorSource, /\* BYOK/, '空状态应保留 BYOK 快捷入口');
assert.match(editorSource, /\{formData\.api_keys\.length > 0 && \(formData\.api_keys\.length < 12 \|\| forceListMode\) && \(/, '有 key 时应渲染列表底部批量入口');
assert.match(editorSource, /批量导入/, 'OAuth 渠道应保留批量导入入口');
assert.match(editorSource, /批量粘贴/, '普通渠道应保留批量粘贴入口');
assert.match(editorSource, /border-dashed/, '机房模式特殊卡片应使用虚线边框');

// 修改原因：OAuth 批量导入依赖前端解析 JSON 和 zip 文件，缺少 JSZip 会导致 zip 上传入口运行时失败。
// 修改方式：断言依赖已写入 package.json，编辑器按需动态导入 JSZip，并调用后端批量导入端点。
// 目的：确保 .zip 导入链路从依赖到接口调用都存在。
assert.equal(typeof packageJson.dependencies?.jszip, 'string', 'package.json dependencies 应包含 jszip');
assert.match(editorSource, /await import\('jszip'\)/, 'ChannelEditor 应动态导入 JSZip 处理 zip 文件');
...
console.log('channel batch import regression passed');
process.exit(0);
